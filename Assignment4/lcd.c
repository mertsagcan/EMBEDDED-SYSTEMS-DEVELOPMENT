...
TASK(LCD_TASK)
{
    ...
    while(1)
    {
        WaitEvent(LCD_READY_EVENT);
        ClearEvent(LCD_READY_EVENT);
        Lcd_refresh();
    }
    TerminateTask();
}
...
void Delay_LCD_ms(unsigned int delay)
{
  ...
  WaitEvent(LCD_ALARM_EVENT);
  GetEvent(LCD_TASK_ID, &LCDevent);
  if (LCDevent & LCD_ALARM_EVENT)
    ClearEvent(LCD_ALARM_EVENT);
  return;
}
...
void LcdPrintString(const char *s,  unsigned char positionX, unsigned char positionY)
{
  ...
  SetEvent(LCD_TASK_ID, LCD_READY_EVENT);
}
